...
if [ ! -f UNIREF30_READY ]; then
  tar --no-same-owner -xzvf "${UNIREF30DB}.tar.gz"
...
if [ ! -f COLABDB_READY ]; then
  tar --no-same-owner -xzvf "colabfold_envdb_202108.tar.gz"
...
if [ ! -f PDB100_READY ]; then
  tar --no-same-owner -xzvf pdb100_foldseek_230517.tar.gz pdb100_a3m.ffdata pdb100_a3m.ffindex
  touch PDB100_READY
fi
